...
## 🏗️ 当前智力档位: v6.0.0 (Pure Brain Mode · 中央大脑定型)

---

## [v6.0.0] - 2026-03-26（当前：Pure Brain Mode · 中央大脑定型）
...
- **技能母库接入但不并仓**：保留 `~/Desktop/skills` 作为 skills 真源，通过 `skills:sync` 将索引引入 CortexOS，使主脑“认识技能”而不破坏 SSOT。
...
- **规范能力建立**：重大改动不再只依赖即时对话和临时决策，OpenSpec 让“为什么改、怎么改、影响什么”可以被 AI 和人类共同追踪。
- **系统复杂度可控**：skills、subagent、Cloud Brain、前台页面与后台进程虽然都在增长，但开始围绕中央大脑这一单核定义运行，而不是继续平铺叠加。

### 影响
v6.0.0 不是单一功能升级，而是 CortexOS 的一次 **产品身份重构**。  
从这一版开始，CortexOS 不再被理解为某个 agent、某个队伍或某个协作看板，而是 `webkubor` 的中央大脑：负责收消息、做判断、分发任务、沉淀知识，并为外部 agent 网络提供统一的协议入口和决策中枢。这也意味着后续所有增长，都应围绕“中央大脑”展开，而不是回到“多系统混讲”的旧路径。
