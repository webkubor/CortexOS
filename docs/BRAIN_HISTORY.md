...
## v7.1.0 — 初始化收口与本地 API 回归（2026-04-01 · 当前）

### 问题

v7.0.0 虽然完成了前端瘦身与 Cloud Brain 收口，但真正落到日常使用时，又暴露出四个“冷启动现实问题”：

1. **初始化不是一把梭**：仓库里有 `bin/cortexos`，但 shell 里没有 `cortexos` 命令；文档要求先 `cortexos brief`，实际新环境却会直接报 `command not found`。

2. **本地 API 缺少稳定入口**：`services/brain-api` 只被写成了 Cloud Run 扩展层，本地没有常驻托管与统一探活链路。结果是：有代码，但不是“本地大脑的一部分”。

3. **默认端口选择过于脆弱**：本地 `brain-api` 使用 `8080`，这在开发机上是高冲突端口，导致服务表面上“能启动”，实际却经常因端口被占用而随机失败。

4. **运维闭环不完整**：通知可以新增、查询、分诊，但不能通过 API 删除；测试通知清理只能直连 Firestore，运维动作没有回收到主脑接口层。

### 思考

我意识到 v7.0.0 过度强调了“去复杂化”，却把一个关键现实忽略了：

> **删掉旧系统，不等于新系统已经形成可用的默认路径。**

真正的 Harness 不是“把东西都删掉”，而是让最常用的启动、探活、清理、查看状态，都有稳定默认值。
...
- **一键初始化收口**：把 `scripts/core/init-project.sh` 收口成真正可用的 `pnpm bootstrap`，自动完成 CLI 命令安装、依赖同步、知识入库、PM2 启动、本地 API 健康检查。

- **本地 Brain API 常驻化**：在 `ecosystem.config.cjs` 中加入 `brain-api-local`，由 `PM2` 托管 `services/brain-api/src/index.js`，使其成为本地主脑的正式入口之一。

- **端口策略重定型**：本地默认端口从 `8080` 改为 `3679`，并支持 `BRAIN_API_PORT` 覆盖，减少开发机端口冲突。

- **状态面板补齐本地视角**：`pnpm brain:status` 开始同时显示云端大脑、本地 API、本地 API 进程三类状态，不再只看 Cloud Run。

- **依赖清单修复**：补齐 `ollama` Python 包依赖，解决 `uv sync` 成功但 `chroma_ingest.py` 首次执行失败的问题。
...
真正好的主脑架构，不是“什么都没有”，而是只保留最少但完整的一条默认路径。  
`bootstrap -> cortexos brief -> brain-api-local -> Cloud Brain`，这条链现在才算真正闭环。
