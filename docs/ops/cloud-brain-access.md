...
详细请求字段、返回结构与完整 `curl` 示例，请看：

- [brain-api](./brain-api)

## 当前主脑项目 ID

当前这套 Cloud Brain 默认服务的主脑项目就是：
...
1. 显式传参时，`project` 建议统一写 `cortexos`
2. 如果调用方没传 `project`，后端现在会自动补成 `cortexos`
3. 同时兼容 `projectId / project_id` 这种写法
...
> 当前线上部署的 `authMode` 仍是 `disabled`，所以暂时不强制 token。  
> 这份文档保留的是“收口后应采用的规范接入方式”。
