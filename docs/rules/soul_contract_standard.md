...
1. 用户资产记忆（User Memory）：
   - 路径：`../memory/knowledge`、`../memory/secrets`、`docs/memory/logs`
   - 内容：用户项目知识、复盘、操作留痕、凭证模板与索引
2. 助手私有调教记忆（Assistant Private Memory）：
   - 路径：`$CODEX_HOME/.memory`
   - 内容：助手偏好、提示策略、思维习惯、调参备忘、失败重试模式
3. 红线：
...
2. 助手个人偏好、语气调教、元提示实验记录，默认仅写 `$CODEX_HOME/.memory`。
3. 不确定是否应写入用户 `memory` 时，先询问用户。
