...
新项目必须包含以下四个环境文件，并加入 `.gitignore`（只提交 `.env.example`）：

| 文件名 | Git 状态 | Vite 加载时机 |
|--------|------------|---------------|
| `.env.development` | 忽略 | `pnpm dev` (mode: development) |
| `.env.test` | 忽略 | `pnpm build:test` (mode: test) |
| `.env.production` | 忽略 | `pnpm build` (mode: production) |
| `.env.example` | **提交** | 作为团队成员的环境变量参考模板 |
...
#------------------------------------------------------------------
# AI 前端项目环境变量模板
# 首次拉取项目后，拷贝本文件到 .env.development 并补全本地开发值
#------------------------------------------------------------------
...
# API 请求地址
VITE_APP_PROXY_URL=/proxy
VITE_APP_BASE_URL=http://your-dev-api.com

# 路由模式 (hash | history)
VITE_APP_ROUTER_MODE=history

# 联调专用，绕过 Mock
VITE_APP_MOCK_DISABLED=true
```
...
`vite build --mode test` 会加载并覆盖：
`.env` (所有环境共享) → `.env.test` (测试环境专用)

#### 4. 变量命名与使用规范

- **前缀**：所有自定义环境变量必须以 `VITE_` 开头，否则在客户端代码中无法通过 `import.meta.env` 访问。
- **应用级变量**：推荐使用 `VITE_APP_` 作为二级前缀，与 Vite 内置变量区分，例如 `VITE_APP_TITLE`。
- **类型与访问**：通过 `import.meta.env.VITE_APP_XXX` 访问到的值都是字符串。如果是布尔值，要手动比较：`import.meta.env.VITE_APP_MOCK_DISABLED === 'true'`。
- **禁止**：不要在 `.env` 文件中定义 `NODE_ENV`，Vite 会根据 `--mode` 自动设置。

#### 5. 新增环境变量流程

1. 在 `.env.example` 中新增变量并加注释说明用途。
2. 在 `.env.development` / `.env.test` / `.env.production` 中补齐对应环境的值。
3. 通知团队成员同步更新本地环境。

### 构建脚本

- 本地开发：`pnpm dev`  (vite)
- 测试环境构建：`pnpm build:test` (vite build --mode test)
- 生产环境构建：`pnpm build` (vite build --mode production)
- 如果项目需要先做类型检查，允许扩展为 `pnpm type-check && vite build`。
